...
// 	init_heatmap(heatmap, test_data_2_ptr, 20, 20);
	...
	init_heatmap(heatmap, NULL, 0, 0);

	// unsigned char* test_data_3 = calloc(20 * 20, sizeof(unsigned char));
	// for (int i = 0; i < 20; i++) {
	// 	for (int j = 0; j < 20; j++) {
	// 		test_data_3[i * 20 + j] = rand() % 255;
	// 	}
	// }
	// init_heatmap(heatmap, test_data_3, 20, 20);

	// const int heatmap_width = 200, heatmap_height = 100;
	// unsigned char* test_data_4 = calloc(heatmap_width * heatmap_height, sizeof(unsigned char));
	// for (int i = 0; i < heatmap_height; i++) {
	// 	for (int j = 0; j < heatmap_width; j++) {
	// 		test_data_4[i * heatmap_width + j] = rand() % 255;
	// 	}
	// }
	// init_heatmap(heatmap, test_data_4, heatmap_width, heatmap_height);

	// const int heatmap_width = 20, heatmap_height = 10;
	// const int size_increase = 200;
	// size_t heatmap_size = heatmap_width * heatmap_height * size_increase * size_increase;
	// unsigned char* test_data_5 = calloc(heatmap_size, sizeof(unsigned char));
	// for (int i = 0; i < heatmap_height; i++) {
	// 	for (int j = 0; j < heatmap_width; j++) {
	// 		test_data_5[i * heatmap_width * size_increase + j] = rand() % 255;
	// 	}
	// }
	// init_heatmap(heatmap, test_data_5, heatmap_width * size_increase, heatmap_height * size_increase);
}
